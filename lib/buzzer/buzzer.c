...
void atualizar_buzzer_alarme(uint buzzer_pin) {
...
void buzzer_confirmacao(uint buzzer_pin) {
    ...
    for (int i = 0; i < 3; i++) {
        buzzer_on(buzzer_pin, freq_inicial + (i * 100), duracao_bip);
        sleep_ms(intervalo);
    }
}
...
void bipe_inicio_gravacao(uint gpio) {
...
void bipe_gravacao_concluida(uint gpio) {
